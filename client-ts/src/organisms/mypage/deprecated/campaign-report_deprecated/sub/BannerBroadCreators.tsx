...
export default function BannerBroadCreators(props: BannerBroadCreatorsProps): JSX.Element {
  ...
  const handleClick = (event: React.MouseEvent<HTMLElement, MouseEvent>, index: number): void => {
    ...
    if (creatorsData.data) {
      ...
      // 수정필요.
      axios
        .get(`${HOST}/creators/analysis/detail-data`, {
          params: { creatorId },
  ...
  return (
    <div {...rest}>
      {creatorsData.data && creatorsData.data.length === 0 ? null : (
          ...
          <Grid container style={{ height: 380, overflow: 'auto' }} id="broad-creators">
            <Grid item container direction="column">
              <Typography variant="caption">
                * 송출량 순 상위 50명까지의 방송인 목록입니다.
              </Typography>
              <Typography variant="caption">
                * 방송인 클릭 시, 상세정보를 볼 수 있습니다.
              </Typography>
            </Grid>
            {creatorsData.data &&
              creatorsData.data.slice(0, 50).map((creator: CreatorDataInterface, index: number) => (
                <Grid key={creator.creatorId} item xs={4} md={3} lg={2} style={{ padding: 8 }}>
                  ...
                  <Typography variant="body1">
                    {`${index + 1}. ${creator.creatorTwitchName || creator.afreecaName}`}
                  </Typography>
                </Grid>
              ))}
