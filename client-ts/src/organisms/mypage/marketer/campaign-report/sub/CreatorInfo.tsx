...
const useStyles = makeStyles((theme) => ({
  container: {
    padding: theme.spacing(2), minWidth: 600
  },
  spaceBetween: {
        ...
        {!empty ? (
          <Grid container direction="column" style={{ marginTop: 20 }}>
              ...
              <Grid container direction="column">
                <Grid item>
                  <Grid container direction="row" className={classes.flex} spacing={1}>
                    ...
                    <Grid item xs={2}>
                      {makeValueComponent({ value: creatorInfo.viewer || creatorInfo.viewerAfreeca, unit: '명' })}
                    </Grid>
                  </Grid>
                  <Grid container direction="row" className={classes.flex} spacing={2}>
                    ...
                    <Grid item xs={2}>
                      {makeValueComponent({ value: creatorInfo.airtime || creatorInfo.airtimeAfreeca, unit: '분' })}
                    </Grid>
                    <Grid item xs={3}>
                      {makeNameComponent({ value: '평균 방송 시간대' })}
                    </Grid>
                    <Grid item xs={3}>
                      {makeValueComponent({ value: creatorInfo.openHour || creatorInfo.openHourAfreeca, unit: '' })}
                    </Grid>
                  </Grid>
                  <Grid container direction="row" className={classes.flex} spacing={2}>
                    <Grid item xs={3}>
                    ...
                    <Grid item xs={2}>
                      {makeValueComponent({ value: creatorInfo.impression || creatorInfo.impressionAfreeca, unit: '' })}
                    </Grid>
                    <Grid item xs={3}>
                      {makeNameComponent({ value: '팔로워 수' })}
                    </Grid>
                    <Grid item xs={2}>
                      {makeValueComponent({ value: creatorInfo.followers || creatorInfo.followersAfreeca, unit: '명' })}
                    </Grid>
                  </Grid>
                </Grid>
              </Grid>
            ...
            <Grid item>
              <ContentsPie
                selectedChartData={
                  JSON.parse(creatorInfo.contentsGraphData || creatorInfo.contentsGraphDataAfreeca)
                }
              />
            </Grid>
            <Grid>
              <TimeChart
                selectedChartData={
                  JSON.parse(creatorInfo.timeGraphData || creatorInfo.timeGraphDataAfreeca)
                }
              />
            </Grid>
          </Grid>
