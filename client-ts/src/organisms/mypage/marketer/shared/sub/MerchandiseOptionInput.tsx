...
import {
  Add, ArrowDownward, ArrowUpward, Close
...
export interface MerchandiseOptionInputProps {
  ...
  optionOperations: Pick<SwapableListItemResult<MerchandiseOption>,
    'handleChange' | 'addItem' | 'downItemPosition' | 'removeItem' | 'upItemPosition' | 'handleReset'
  >;
...
export default function MerchandiseOptionInput({
  ...
  return (
    <div className={classes.container}>
      ...
      {options.map((opt, idx1) => ({ ...opt, id: idx1 + 1 })).map((option, idx) => (
      ...
      <Button
        variant="contained"
        size="small"
        onClick={() => optionOperations.addItem({
          type: '',
          name: '',
          additionalPrice: ''
        })}
