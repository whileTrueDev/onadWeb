...
import renderMerchandiseUploadState, {
  MERCHANDISE_UPLOAD_SOLDOUT,
  MERCHANDISE_UPLOAD_WAITING,
} from '../../../../../../utils/render_funcs/renderMerchandiseUploadState';
...
export default function CampaignDetail({ campaign }: CampaignDetailProps): React.ReactElement {
   ...
   */
  const renderConfirmState = useCallback(
    (type: '배너' | '랜딩URL', confirmState: number): React.ReactElement => (
    ...
    <>
          ...
          <Alert severity="error">
            <Typography variant="body2">
              {`캠페인 ${campaign.campaignName} 의 상품 -> ${withJosa(
                campaign.merchandiseName,
                '이/가',
              )} 아직 온애드샵에 업로드되지 않아 광고 진행이 불가한 상태입니다.`}
            </Typography>
        ...
        <Alert severity="info">
          ...
          <Typography variant="body2">
            {`캠페인 ${campaign.campaignName} 의 상품 -> ${withJosa(
              campaign.merchandiseName,
              '이/가',
            )} 재고 소진으로 인해 더이상 광고 진행이 불가한 상태입니다.`}
          </Typography>
          <Typography variant="body2">
            더 많은 상품 판매를 원하신다면 새로 상품을 등록하고 캠페인을 등록해주세요.
          </Typography>
        </Alert>
      ) : null}
      ...
      <article className={classes.article}>
        ...
        {campaign.linkId && campaign.linkConfirmState && (
          <span>{renderConfirmState('랜딩URL', campaign.linkConfirmState)}</span>
        )}
        {campaign.merchandiseId && (
          <>
            <Typography>
              {`상품 판매 상태 - ${renderMerchandiseUploadState(
                campaign.merchandiseUploadState || null,
              )}`}
        ...
        {campaign.targetCreators ? (
          <div>
            <Typography color="textSecondary" variant="body2">
              방송인 클릭시, 해당 방송인의 채널/방송국으로 이동합니다.
            </Typography>
                    ...
                    <ButtonBase
                      ...
                      onClick={(): void => {
                        if (creator.afreecaId)
                          window.open(`http://play.afreecatv.com/${creator.afreecaId}`);
                        else if (creator.creatorTwitchId)
                          window.open(`https://twitch.tv/${creator.creatorTwitchId}`);
                      }}
                    >
