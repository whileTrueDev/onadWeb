      ...
      {MainUserType ? (
        <Slider {...silderProps} MainUserType={MainUserType}>
          {source.marketer.map((content: any, i: number) => (
            <div className={classes.contentWrapper} id={`contentWrap${i}`} key={shortid.generate()} onClick={(e) => flipHandler(e)}>
              <div className={classNames({ [classes.figure]: !itemClicked[`contentWrap${i}`], [classes.flipFront]: itemClicked[`contentWrap${i}`] })}>
              ...
              <div className={classNames({ [classes.caption]: !itemClicked[`contentWrap${i}`], [classes.flipBack]: itemClicked[`contentWrap${i}`] })}>
                <Typography variant="h4" className={classes.title}>{content.title}</Typography>
          ...
          <Slider {...silderProps} MainUserType={MainUserType}>
            {source.creator.map((content: any, i: number) => (
              <div className={classes.contentWrapper} id="contentWrap" key={shortid.generate()}>
                <div className={classNames(classes.figure, { [classes.flipFront]: itemClicked[`contentWrap${i}`] })}>
                  <img src={content.imageUrl} alt="advantage" className={classes.advImg} />
                  <Typography variant="h4" className={classes.title}>{content.title}</Typography>
                  <Typography variant="subtitle2" className={classes.titleBottom}>자세히보기 &gt;</Typography>
                </div>
                <div className={classNames(classes.caption, { [classes.flipBack]: itemClicked[`contentWrap${i}`] })}>
                  <Typography variant="h4" className={classes.title}>{content.title}</Typography>
                  <Divider className={classes.divider} />
                  <Typography variant="subtitle1">{content.content}</Typography>
                </div>
