...
export default function IndentityVerification({
  ...
  useEffect(() => {
    if (iamportDialogOpen) {
      ...
      IMP.certification({
        merchant_uid: 'ORD20180131-0000011', min_age: '19'
      }, (rsp: any) => { // callback
        if (rsp.success) {
  ...
  return (
    <div className={classes.contents}>
      ...
      <div className={classes.contents}>
        <Typography variant="body2">자동가입 및 중복 가입 방지를 위해 본인인증 절차가 필요합니다.</Typography>

        <div className={classes.contents}>

          {helperText && (<Typography color="error" variant="body2" className={classes.helperText}>{helperText}</Typography>)}
          {success && (<Typography className={classes.helperText}>본인인증에 성공했습니다!</Typography>)}
          <Button
