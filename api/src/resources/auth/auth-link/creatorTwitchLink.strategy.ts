...
@Injectable()
export class TwitchLinkStrategy extends PassportStrategy(Strategy, 'twitch-link') {
  constructor(
    ...
    super({
      ...
      callbackURL: `${configService.get<string>('API_HOSTNAME') ||
        'http://localhost:3000'}/link/twitch/callback`,
  ...
  async validate(
    ...
    const { creatorId } = req.user as CreatorSession;
    return this.authService.twitchLink(creatorId, refreshToken, profile);
  }
}
