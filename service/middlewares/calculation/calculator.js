...
const getcreatorList = ({ startDate, endDate }) => {
  const listQuery = `
  ...
  return new Promise((resolve, reject) => {
    doQuery(listQuery, [endDate, startDate])
      .then((row) => {
        const creators = [];
        row.result.map((creator) => {
          creators.push(creator);
        });
        console.log('해당시점에 방송중인 인원: ', creators.length);
        resolve(creators);
      })
      .catch((errorData) => {
...
// list로 전달되는 시간.
// 커넥션 하나물고 for 문을 돌려보자.
const getviewer = ({
  connection, creatorId, startDate, endDate
}) => {
  // 9시간을 제한 값을 통해 streamId를 들고올 수 있다.
  const streamIdQuery = `
  SELECT streamId, startedAt
  FROM twitchStream 
  WHERE streamerId = ? 
  AND startedAt <= CONVERT_TZ(?,'+00:00','+00:00')
  ...
  return new Promise((resolve, reject) => {
    connection.query(streamIdQuery, [creatorId, startDate], (err1, result) => {
      if (err1) {
        reject(err1);
      }
      const { streamId } = result[0];
      connection.query(viewerQuery, [streamId, endDate, startDate], (err2, result1) => {
        if (err2) {
          reject(err2);
        }
...
const getStreamList = ({
  connection, creatorList, startDate, endDate
}) => Promise.all(
...
const getCalculationList = async (time) => {
  const endDate = new Date(time);
  const startDate = new Date(time);
  endDate.setMinutes(startDate.getMinutes() + 10);

  const creatorList = await getcreatorList({ startDate, endDate });
  // try - catch 필요.
  const streamList = await connectionWarp({ func: getStreamList, params: { creatorList, startDate, endDate } });
...
const waitTimePromise = () => new Promise((resolve, reject) => {
...
async function calculation() {
  const time = '2019-11-27 22:45:00';
  console.log(`크리에이터 인원을 계산합니다. 시작 시각 : ${new Date().toLocaleString()}`);
  const calculateList = await getCalculationList(time);
  console.log('해당시점에 배너 게시 중인 인원: ', calculateList.length);
  ...
  Promise.all(
    ...
    .then(waitTimePromise)
    .then(zeroCalculateConnectionWarp({ marketerList }))
    .then(() => {
