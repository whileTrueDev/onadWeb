...
function getStepContent(step, typeChange, handleNext, handleBack, userType, handleUserInfo) {
  switch (step) {
    ...
    case 1:
      return <RegistForm userType={userType } handleNext={handleNext} handleBack={handleBack} handleUserInfo={handleUserInfo}/>;
    case 2:
      return <PaperSheet/>
    default:
      return 'Unknown step';
        ...
        <Stepper activeStep={activeStep} orientation="vertical">
          {steps.map((label, index) => (
            <Step key={label}>
              ...
              <StepContent>
                {getStepContent(index, this.typeChange, this.handleNext, this.handleBack, this.state.userType, this.handleUserInfo)}
              </StepContent>
            </Step>
          ))}
        </Stepper>
