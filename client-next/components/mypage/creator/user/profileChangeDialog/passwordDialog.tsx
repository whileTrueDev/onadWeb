...
export default function PasswordDialog({
    ...
    _passwordCheck
      ...
      .catch(() => {
        setPasswordCheckFail(
          '비밀번호 확인 과정에서 오류가 발생했습니다. 잠시후 다시 시도해주세요.',
        );
      });
  }
    ...
    updatePasswordMutation
      ...
      .then(res => {
        if (res.data > 0) {
          ...
          setChangeFailError(
            '비밀번호 변경 과정에서 오류가 발생했습니다. 잠시후 다시 시도해주세요!',
          );
        }
      })
      .catch(() => {
  ...
  return (
    <CustomDialog fullWidth maxWidth="xs" open={open} onClose={onClose} title="비밀번호 변경">
      {activeStep === 0 && (
        <div>
          <Typography variant="body2" color="textSecondary">
            보안 인증을 위해 현재 비밀번호를 입력해주세요!
          </Typography>
          {/* 비밀번호 틀림 / 오류 알림 */}
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <Button style={{ marginLeft: 4 }} onClick={handleDialogClose} variant="contained">
              취소
            </Button>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ marginTop: 16, marginBottom: 16 }}>
            <TextField
              ...
              helperText={
                newPwCheck && newPw.value !== newPwCheck.value
                  ? '입력한 두 비밀번호가 다릅니다.'
                  : ''
              }
