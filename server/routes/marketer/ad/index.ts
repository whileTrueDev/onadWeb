...
router.route('/on-off')
    ...
    .post(
        ...
        responseHelper.middleware.withErrorCatch(async (req, res, next) => {
            const { marketerId } = responseHelper.getSessionData(req);
            const onOffState = responseHelper.getParam('onOffstate', 'GET', req);
            ...
            doQuery(costQuery, [marketerId])
                .then((row) => {
                    ...
                    if (debit === 0) {
                        responseHelper.send([false, '잔액이 부족합니다'], 'get', res);
                    } else {
                        ...
                        Promise.all([
                            ...
                            .then(() => {
                                responseHelper.send([true, { onOff: contractionState }], 'get', res);
                            })
                            .catch((error) => {
                                responseHelper.promiseError(error, next);
                            });
