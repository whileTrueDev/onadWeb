...
router.route('/testcharge')
  .post(responseHelper.middleware.withErrorCatch(async (req, res, next) => {
    const { marketerId } = responseHelper.getSessionData(req);
    const [
      chargeCashString, chargeType, imp_uid, merchant_uid
    ] = responseHelper.getParam(['chargeCash', 'chargeType', 'imp_uid', 'merchant_uid'], 'post', req);
    const chargeCash = Number(chargeCashString);
    ...
    if (parseInt((chargeCash * 1.1).toString(), 10) === parseInt(amount, 10)) {
      switch (status) {
        case 'ready':
          // 가상계좌 발급 시 로직
          // DB에 가상계좌 발급 정보 저장
          ...
          // 충전 금액 row 한 줄 추가
          const vbankCashChargeArray = [
            marketerId, chargeCash, chargeType,
                  ...
                  .then((secondrow) => {
                    ...
                    if (!secondrow.error) {
                      ...
                      notification({
                        ...
                        targetId: marketerId,
                        params: {
                          cashAmount: amount, vbankName: vbank_name, vbankNum: vbank_num,
                        },
                        vbankDate: vbank_date
                      });
                    }
            ...
            .then((row) => {
              if (!row.error) {
                ...
                Promise.all([
                  ...
                  .then((secondrows) => {
                    console.log(secondrows); // 마케터 캐시 충전 쿼리 완료
                    if (secondrows.findIndex((r) => r.error) === -1) {
      ...
      responseHelper.promiseError(Error('위조된 결제시도!!!'), next);
    }
  }));
