    ...
    Promise.all([
      ...
      .then((rowList) => {
        if (!rowList[0].error && !rowList[1].error) {
          ...
          const marketerIds = marketerInfo.map((marketer: { marketerId: string }) => marketer.marketerId);
...
const scheduler = schedule.scheduleJob(
  'Marketer taxbill scheduler', '1 10 1 * *', process.env.NODE_ENV === 'production' ? () => {
    ...
    updateFail()
      .then(() => {
        slack('마케터 세금계산서 미발행처리 완료.', '마케터 세금계산서');
        ...
        getInsertData().then((data) => {
          ...
          doQuery(q.queryString, q.queryArray).then(() => {
            slack('마케터 세금계산서 데이터 Insert 완료.', '마케터 세금계산서');
          }).catch((err) => {
            console.log(err);
            slack('마케터 세금계산서 데이터 Insert 중 오류발생함.', '마케터 세금계산서');
          });
        });
      })
      .catch((err) => {
