...
const getcreatorList = ({ date }) => {
  ...
  campaign
  ON RT.campaignId = campaign.campaignId
  WHERE NOT campaign.limitState = 1 AND campaign.optionType = 3
  GROUP BY creatorId`; // CPS배너광고(optionType=3)의 경우만 목록으로 불러온다.
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        Promise.all([
          ...
          doConnectionQuery({ connection, queryState: cpsListQuery, params: [date] }),
        ])
          .then(([
            bannerListData,
            offListData,
            ...
            // CPS에 대한 계산 대상 만들기.
            const cpsList = cpsListData.reduce((result, bannerData) => {
              // 1. 배너 송출 목록에 있는 경우
              // 2. offList에 존재하지 않을 경우, 두 조건을 모두 충족시에만 계산항목으로 들어가도록 추가.
              if (uniqueStreamers.includes(bannerData.creatorId)
                && !offList.includes(bannerData.creatorId)) {
...
// *************************************************************
// CPS에 대한 campaignLog 추가 이 때, cash관련 값들은 모두 0을 사용
// @주석작성  chanuuuuu 2021.05.20
// *************************************************************
const cpsCampaignLog = async ( cpaList ) => {
  // cpaList에 대한 campaignLog 추가하기
  if (!Array.isArray(cpaList) || cpaList.length === 0) {
    return;
  }

  let valueText = cpaList.reduce((query, {campaignId, creatorId, viewer, twitchViewer, afreecaViewer}) => {
    const insertText = `('${campaignId}', '${creatorId}', 'CPM', 0, 0, ${viewer}, ${twitchViewer}, ${afreecaViewer}),`
    return query  + insertText;
  }, '');
  ...
  VALUES ${valueText}`;
   ...
   .then(()=> {
      console.log(`CPS campaignLog 입력 완료 : ${cpaList.length}개`)
   })
   .catch((reason)=> {
      console.log(`CPS campaignLog 입력 실패 : ${reason.error}`);
   })
};
...
async function getList(date) {
  // 계산할 대상을 탐색하는 함수.
  const [ creatorList, cpsList ] = await getcreatorList({ date });
  ...
  // CPS로 추가할 대상의 방송정보 (시청자수, cashToCreator, cashFromMarketer)
  const cpsStreamList = await connectionWarp({ func: getStreamList, params: { creatorList: cpsList } });

  // CPS에 대한 campaignLog 추가
  await cpsCampaignLog(cpsStreamList);
