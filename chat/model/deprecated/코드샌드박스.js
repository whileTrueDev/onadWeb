...
Clips
  Endpoint: https://api.twitch.tv/helix/clips/ReliableSplendidInternPogChamp?on_site=1&api_version=5
...
const helixClientID = 'o6x02nuufgjlsv28bywzygfid5uzbu';
...
function addListeners() {
  ...
  function handleMessage(channel, userstate, message, fromSelf) {
    ...
    userstate.name = name;
    showMessage({
      chan, type: 'chat', message, data: userstate
    });
  }
...
function showMessage({
  chan, type, message = '', data = {}, timeout = 0, attribs = {}
} = {}) {
  ...
  if (type === 'chat') {
    ...
    if ('badges' in data && data.badges !== null) {
      ...
      const badgeGroup = Object.assign({}, twitchBadgeCache.data.global, twitchBadgeCache.data[chan] || {});
      const badges = Object.keys(data.badges)
        .forEach((type) => {
...
function addEmoteDOM(ele, data) {
  data.forEach((n) => {
    ...
    if (typeof n === 'string') {
      ...
      const { type: [ type, subtype ], code } = n;
      if (type === 'twitch') {
        if (subtype === 'emote') {
...
function request({
  base = '', endpoint = '', qs, headers = {}, method = 'get'
}) {
  const opts = {
    method,
